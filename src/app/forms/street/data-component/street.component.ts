...
@Component({
    selector: 'street-comp',
    templateUrl: 'street.component.html',
    providers: [HttpService,HttpMessageShow]
    
})
export class StreetComponent extends BaseDataComponent implements EditableDataFormModel{ 
    constructor(public dialog: MatDialog, private httpService: HttpService) {
      super('/street');
      super.setEditForm(this);
      this.gridConfiguration.columns.push(new GridDisplayedColumns("name","Название"));
    ...
    addOrUpdateFormAsync(paramEdit: EditItemsParam) : Promise<ChangeResult>
    {
      const promise = new Promise<ChangeResult>((resolve, reject) => {
        const dialogRef = this.dialog.open(StreetEditComponent, {
          data: paramEdit   
        });
